...
export default function GeneralFaq() {
  ...
  const generalFaq = history.location.state[history.location.state.length - 1];
  return (
    <Grid container className="flex-grow bg-pageBg">
      <Container maxWidth="xl">
        <Grid container direction="column" className="pt-16 pb-60">
          ...
          <Grid item className="w-full">
            {
              generalFaq.length === 0 && <Typography variant="h6" align="center" color="primary">No faqs added yet</Typography>
            }
            {
              generalFaq.length !== 0 && generalFaq.map((faq) => (
